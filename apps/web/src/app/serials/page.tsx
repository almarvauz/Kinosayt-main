...
async function SeriesGrid({
  ...
  try {
    ...
    return <div className="text-center py-20 text-[rgb(var(--muted))]">Seriallarni yuklashda xatolik yuz berdi.</div>;
  }

  if (!result.data.length) {
    return <div className="text-center py-20 text-[rgb(var(--muted))]">Hech qanday serial topilmadi.</div>;
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">
          Seriallar <span className="text-base font-normal text-[rgb(var(--muted))]">({result.total} ta)</span>
        </h1>
      </div>
      ...
      {result.totalPages > 1 && (
        <div className="mt-8 flex justify-center gap-2">
          {Array.from({ length: Math.min(result.totalPages, 10) }, (_, i) => {
            ...
            return (
              <a key={p} href={`/serials?${params.toString()}`}
...
export default function SerialsPage({ searchParams }: { searchParams: Record<string, string | undefined> }) {
  return (
