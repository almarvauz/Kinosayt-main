...
export function R2ImageUploader({ onUploadComplete, label = "Rasm fayl" }: Props) {
  ...
  async function handleFile(file: File) {
    if (!file.type.startsWith("image/")) {
      setState({ status: "error", progress: 0, error: "Faqat rasm fayllar qabul qilinadi" });
      return;
    }
    ...
    <div className="space-y-2">
      ...
      {state.status === "idle" || state.status === "error" ? (
        <>
          ...
          >
            <ImageIcon className="w-8 h-8 text-[rgb(var(--muted))] mb-2" />
            <p className="text-sm text-[rgb(var(--muted))]">
              Bosing yoki rasmni bu yerga torting
            </p>
            <p className="text-xs text-[rgb(var(--muted))]/60 mt-1">JPEG, PNG, WEBP — max 5MB</p>
          </div>
          <input
            ref={inputRef}
            type="file"
